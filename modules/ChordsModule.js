...
export default class ChordsModule extends AbstractRackModule {
    constructor(context, params) {
        ...
        this.addPlug(stepTriggerParam, '', 'in', null, 'fixed', { x: 30, y: 40 });
        const stepLabel = {
            getText: () => 'Step',
            position: { x: 50, y: 43 },
            align: 'left',
        ...
        this.addPlug(resetTriggerParam, '', 'in', null, 'fixed', { x: 120, y: 40 });
        const resetLabel = {
            getText: () => 'Reset',
            position: { x: 140, y: 43 },
            align: 'left',
        ...
        for (let y = 0; y < 8; y++) {
            ...
            this.addPlug(this.noopGain, '', 'in', null, 'fixed', { x: 150, y: this.gridYOffset + (this.gridYSpan * y) });
        }
        for (let x = 0; x < 4; x++) {
            this.addPlug(this.channelOutputs[x], '', 'out', null, 'fixed', { x: this.gridXOffset + (this.gridXSpan * x), y: 380 });
        }
